...
async def process_prices_updates(prices: dict):
    ...
    async def process_price(market_address, price):
        ...
        down_results = users.find(
            {
                f"notification_configs.{market_address}.threshold_price": {"$gt": price},
        ...
        if down_results:
            for result in down_results:
                ...
                market_chain = result['notification_configs'][market_address]['market_chain'].upper()
                threshold_price = result['notification_configs'][market_address]['threshold_price']
                await send_message(f"⚠️ Price alert for your Defimoney position on {market_chain}.\n{market_name} is now at ${round(price, 2)}.\nYou are configured to receive price alert when {market_name} drops below ${threshold_price}", result['tg_id'])
                query_filter = {"tg_id": result['tg_id']}
                notification_configs_for_market = result['notification_configs'][market_address]
                notification_configs_for_market['has_price_crossed_threshold'] = True
                update_operation = {"$set": {f"notification_configs.{market_address}": notification_configs_for_market}}
        ...
        up_results = users.find(
            {
                f"notification_configs.{market_address}.threshold_price": {"$lt": price},
    ...
    tasks = [process_price(market_address, price) for market_address, price in prices.items()]
...
async def poll_oracles_task(queue):
    while True:
        # LATEST_PRICES = poll_oracles()
        prices = poll_oracles()
        # inspect(LATEST_PRICES, log_locals=True)
        # print(f'LATEST_PRICES: \n{LATEST_PRICES}')
