...
def poll_oracles():
    arbitrum_markets = [
        market_address for market_address in arbitrum_markets_configs.keys()
    ]
    optimism_markets = [
        market_address for market_address in optimism_markets_configs.keys()
    ]

    price_oracle_calldata = "0x86fc88d3"

    arbitrum_amm_addresses = [
        price["amm_address"] for price in arbitrum_markets_configs.values()
    ]
    optimism_amm_addresses = [
        price["amm_address"] for price in optimism_markets_configs.values()
    ]
    ...
    arbitrum_call_res = arbitrum_multicall_contract.functions.aggregate3(
        arbitrum_calls
    ).call()
    optimism_call_res = optimism_multicall_contract.functions.aggregate3(
        optimism_calls
    ).call()

    arbitrum_prices_decoded = [
        decode(["uint256"], call[1]) for call in arbitrum_call_res
    ]
    optimism_prices_decoded = [
        decode(["uint256"], call[1]) for call in optimism_call_res
    ]
...
def get_market_states_for_account(account_address: str):
    ...
    arbitrum_account_states = (
        arbitrum_cdp_market_views_contract.functions.get_market_states_for_account(
            account_address, arbitrum_markets
        ).call()
    )
    optimism_account_states = (
        optimism_cdp_market_views_contract.functions.get_market_states_for_account(
            account_address, optimism_markets
        ).call()
    )
    ...
    for market in arbitrum_account_states:
        ...
        market_collateral_decimals: int = arbitrum_markets_configs[market_address][
            "collateral_decimals"
        ]
    ...
    for market in optimism_account_states:
        ...
        market_collateral_decimals: int = optimism_markets_configs[market_address][
            "collateral_decimals"
        ]
